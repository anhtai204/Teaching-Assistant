    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-[#1A1A3A] border border-slate-100 dark:border-white/5 rounded-2xl shadow-2xl z-20 py-2 animate-in fade-in zoom-in-95 duration-200">
            <div className="px-4 py-3 border-b border-slate-50 dark:border-white/5 mb-2">
              <p className="text-xs font-bold text-slate-400 dark:text-white/40 uppercase tracking-widest mb-1">Signed in as</p>
              <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{session.user.email}</p>
            </div>

            <button 
              onClick={toggleTheme}
              className="w-full px-4 py-2 flex items-center gap-3 text-sm font-bold text-slate-600 dark:text-white/70 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors"
            >
            ...
            <div className="h-[1px] bg-slate-50 dark:border-white/5 my-2" />

            <button 
              onClick={() => { setIsLogoutModalOpen(true); setIsOpen(false); }}
              className="w-full px-4 py-2 flex items-center gap-3 text-sm font-bold text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors"
            >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/80 backdrop-blur-md" onClick={() => setIsModalOpen(false)} />
          <Card className="w-full max-w-md p-6 relative shadow-2xl dark:bg-[#1A1A3A] dark:border-white/10 animate-in zoom-in-95 duration-200 z-[71]">
      ...
      {isLogoutModalOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 animate-in fade-in duration-300">
           <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/80 backdrop-blur-md" onClick={() => setIsLogoutModalOpen(false)} />
           <Card className="w-full max-w-sm p-8 relative shadow-2xl dark:bg-[#1A1A3A] dark:border-white/10 animate-in zoom-in-95 duration-200 z-[71]">
