...
import { Send, Info, MessageSquare, History, ArrowLeft, Download, ExternalLink, Flag, ThumbsUp, ThumbsDown, Sparkles, X, FileText, CheckCircle2, PlusCircle, Clock } from "lucide-react";
...
function ChatContent() {
  ...
  const renderContent = (content: string, chunks?: any[]) => {
    ...
    return parts.map((part, i) => {
      ...
      if (match) {
        ...
        return (
          <button
            key={i}
            onClick={() => setPreviewSource(chunk)}
            className="inline-flex items-center justify-center h-6 w-6 bg-blue-100 text-blue-700 text-[11px] font-bold rounded-lg mx-1 hover:bg-blue-600 hover:text-white transition-all border border-blue-200 shadow-sm cursor-pointer"
            title={chunk?.metadata?.source || "View Source"}
          >
            {match[1]}
          </button>
        );
            ...
            <div className="space-y-1">
              {chatSessions.map((s) => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl text-sm transition-all group relative border ${
                    currentSessionId === s.id ? "bg-white dark:bg-[#1A1A3A] shadow-md border-blue-100 dark:border-blue-500/30 text-blue-600 dark:text-blue-400 font-bold" : "text-slate-500 dark:text-white/40 border-transparent hover:bg-white dark:hover:bg-white/5 hover:shadow-sm"
                  }`}
                >
          ...
          <div className="max-w-4xl mx-auto space-y-12 pb-24">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-6 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`h-12 w-12 shrink-0 rounded-2xl flex items-center justify-center font-bold text-xs shadow-md transition-transform duration-300 ${
                  msg.role === "assistant" ? "bg-blue-600 text-white" : "bg-white text-slate-600 border border-slate-100"
                }`}>
                ...
                <div className={`max-w-[85%] space-y-4 ${msg.role === "user" ? "text-right" : ""}`}>
                  <div className={`p-7 rounded-[2rem] text-slate-800 leading-relaxed text-[16px] shadow-sm relative transition-all ${
                    msg.role === "assistant" 
                    ...
                    {renderContent(msg.content, msg.chunks)}
                  </div>
                      ...
                      {msg.sources && msg.sources.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {msg.sources.map((s, si) => (
                            <span key={si} className="text-[10px] font-black text-slate-400 border border-slate-200 px-3 py-1 rounded-full bg-white shadow-sm flex items-center gap-1.5 uppercase tracking-tighter">
